...
import { User, Workout, UserPreferences, WorkoutCategory, Exercise } from './index';
...
export interface UploadTask {
  ...
  cancel(): boolean;
  catch(onRejected: (error: Error) => Promise<never>): Promise<UploadTaskSnapshot>;
  then(
    onFulfilled?: (snapshot: UploadTaskSnapshot) => UploadTaskSnapshot,
...
export interface FirebaseAnalytics {
  logEvent(name: string, properties?: Record<string, string | number | boolean>): void;
...
export const workoutConverter: FirestoreConverter<Workout> = {
  ...
  fromFirestore(snapshot: QueryDocumentSnapshot): Workout {
    ...
    return {
      ...
      category: data.category as WorkoutCategory,
      exercises: data.exercises as Exercise[],
      completed: data.completed as boolean,
      completedAt: data.completedAt ? (data.completedAt as { toDate: () => Date }).toDate() : undefined,
